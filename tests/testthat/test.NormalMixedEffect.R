...
nIter   <- 700
test_that("tdist measurement error", {
  nIter   <- 1000
  nu <- 3
  for(i in 1:n.pers)
  {
    ...
    V <- LDMod::rGIG(rep(-nu,n.obs), rep(0,n.obs), 2 * rep(nu + 1,n.obs), sample.int(10^6,1))
    V_list[[i]] <- V
  ...
  meas_list <- list(sigma_eps = 2*0.1,Vs = V_list, nu = 0.5*nu, noise = "tdist")
